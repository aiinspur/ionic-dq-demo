...
export class HomePage {
  ...
  carouselList: any;//轮播图列表
  articlesList: any;
  resourcesApi = AppConfig.resource_api;
  titleLength: 20;
  carouseLen: any;
  ...
  //获取轮播图
  getCarousel() {
    this.http.get(AppConfig.article_list_api + "?carousel=1").subscribe(
      (result: any) => {
        if (result.code == AppConfig.success) {
          this.carouselList = result.data;
          this.carouseLen = this.carouselList.length;
        }
      },
      response => {
  ...
  getArticles(q?: string) {
    ...
    this.http.get(AppConfig.article_page_api+q).subscribe(
      ...
      () => {
        ...
        //console.log("articlesList:%o",this.articlesList);
      });
  }

  inpage(article) {
    console.log('in page:%d', article.id);
